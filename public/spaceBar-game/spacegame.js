...
const wallup = {
  ...
  draw: function () {
    ...
    ctx.drawImage(
      ...
      this.w,
      this.h,
      this.x + this.wd + this.wd + this.wd,
      this.y,
      this.wd,
      this.hd
    );
...
const lazer = {
  ...
  update: function () {
    ...
    for (let i = 0; i < this.position.length; i++) {
      ...
      // TOP lazer
      if (
        man.x + man.radiusw > p.x &&
        man.x - man.radiusw < p.x + this.wd &&
        man.y + man.radiush > p.y &&
        man.y - man.radiush < p.y + this.hd
      ) {
        state.current = state.over;
        // HIT.play();
      }
      ...
      if (
        man.x + man.radiusw > p.x &&
        man.x - man.radiusw < p.x + this.wd &&
        man.y + man.radiush > bottomPipeYPos &&
        man.y - man.radiush < bottomPipeYPos + this.hd
      ) {
        state.current = state.over;
        // HIT.play();
      }
